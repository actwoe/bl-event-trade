...
function AdminUiLabCreatePageContent() {
  ...
  const eventPeriod = collection
    ? [
        formatDate(collection.eventStartDate),
        formatDate(collection.eventEndDate),
      ]
        .filter(Boolean)
        .join(" ~ ")
    : "";
...
export default function AdminUiLabCreatePage() {
  return (
    <Suspense fallback={<main className="flex h-[100dvh] items-center justify-center bg-neutral-100 text-sm text-neutral-500">교환판 생성 화면을 불러오는 중입니다.</main>}>
